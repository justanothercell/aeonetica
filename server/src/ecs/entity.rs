...
use aeonetica_engine::sha2::digest::typenum::Mod;
...
impl<'a> Entity<'a> {
    ...
    pub fn add_module<T: Module + Sized + 'static>(&'a mut self, module: T) -> bool {
    ...
    pub fn remove_module<T: Module + Sized + 'static>(&'a mut self) {
        self.modules.remove(&TypeId::of::<T>());
    }

    pub fn get_module<T: Module + Sized + 'static>(&'a self) -> Option<&'a T> {
    ...
    pub fn has_module<T: Module + Sized + 'static>(&'a self) -> bool {
        self.modules.contains_key(&TypeId::of::<T>())
    }

    pub fn get_id(&'a self) -> Id {
        self.entity_id
    }
}