...
void main() {
    ...
    for(uint i = 0; i < u_NumLights; i++) {
        v_ToLightVector[i] = vec3(u_LightPositions[i], 1.0) - vec3(a_Position, 0.0);
    }
}
...
void main() {
    ...
    for(uint i = 0; i < u_NumLights; i++) {
        float intentsity = u_LightIntensities[i];
        float dist = length(v_ToLightVector[i]);
        if(dist < intentsity) {
            float inv_dist = -dist + intentsity;
            total_diffuse += (u_LightColors[i] * vec3(inv_dist / intentsity));
        }
    }
